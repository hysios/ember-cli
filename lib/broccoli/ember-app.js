...
EmberApp.prototype.adminIndex = memoize(function() {
  var files = [
    'admin.html'
  ];
  ...
  index = moveFile(index, {
    srcFile: 'admin/admin.html',
    destFile: 'admin/index.html'
  });
...
EmberApp.prototype.sprites = memoize(function() {
  this.spritesTree = BroccoliSpritesmith('public/sprites/*.png')
  return this.spritesTree;
});
...
EmberApp.prototype.styles = memoize(function() {
  ...
  var spritesTree = BroccoliSpritesmith('public/sprites/*.png');

  var spriteScss = pickFiles(spritesTree, {
    srcDir: 'assets/sprites',
    files: ['*.scss'],
    destDir: '/app/styles'
  })

  var trees = [vendor].concat(addonTrees, spriteScss);
  trees.push(styles);
  ...
  var adminStyles = preprocessCss(stylesAndVendor, '/app/styles', '/assets', {name: 'admin', output: 'admin'});
  var vendorStyles = preprocessCss(stylesAndVendor, '/app/styles', '/assets', {name: 'vendor', output: 'vendor'});

  vendorStyles = mergeTrees([vendorStyles, stylesAndVendor]);
  this.vendorStaticStyles.unshift('assets/vendor.css');

  vendorStyles  = concatFiles(vendorStyles, {
  ...
  if (this.env === 'production' && this.options.minifyCSS.enabled === true) {
    ...
    adminStyles = preprocessMinifyCss(adminStyles, options);
